    ...
    def _get_clusters_locations(cls):
        ...
        for location in location_list:
            ...
            if cluster in orch_result:
                # cephadm might have set some cluster settings, ask the user to remove it.
                raise NFSException(
                    'A conflicted NFS-Ganesha cluster name `{0}` is detected. There exists an '
    ...
    def _get_orch_clusters_locations(cls):
        ...
        for service in services:
            ...
            try:
                ...
                logger.warning('Error when getting NFS service from the Orchestrator. %s', ex)
                continue
        return orch_result
...
class GaneshaConf(object):
    ...
    def _read_raw_config(self):
        ...
        def _read_rados_obj(_obj):
            size, _ = _obj.stat()
            raw_config = _obj.read(size)
            return raw_config.decode("utf-8")
...
class GaneshaConfOrchestrator(GaneshaConf):
    ...
    def validate(self, export: Export):
        ...
        if export.daemons and set(export.daemons) != daemons_list:
            raise NFSException('Export need to be linked to no or all daemons.')
