...
from ..services.ganesha import ClusterType, NFSException
    ...
    def test_list_daemons(self):
        for cluster_id, info in self.clusters.items():
            ...
            for daemon in daemons:
                ...
                # Daemons reported by the Orchestrator contain status and status_desc
                self.assertEqual('status' in daemon, info['type'] == ClusterType.ORCHESTRATOR)
                self.assertEqual('status_desc' in daemon, info['type'] == ClusterType.ORCHESTRATOR)
